...
void Spheroid::set(double a_, double f_)
{
  ...
  alpha[1] = // alpha4
    ...
    n4 * ((      557.0) / (     1440.0)) +
    n5 * ((      281.0) / (      630.0)) -
    n6 * ((  1983433.0) / (  1935360.0)) +
    n7 * ((    13769.0) / (    28800.0)) +
    n8 * ((148003883.0) / (174182400.0));

  alpha[2] = // alpha6
    n3 * ((      61.0) / (     240.0)) -
    n4 * ((     103.0) / (     140.0)) +
    n5 * ((   15061.0) / (   26880.0)) +
    n6 * ((  167603.0) / (  181440.0)) -
    n7 * ((67102379.0) / (29030400.0)) +
    n8 * ((79682431.0) / (79833600.0));

  alpha[3] = // alpha8
    n4 * ((      49561.0) / (    161280.0)) -
    n5 * ((        179.0) / (       168.0)) +
    n6 * ((    6601661.0) / (   7257600.0)) +
    n7 * ((      97445.0) / (     49896.0)) -
    n8 * ((40176129013.0) / (7664025600.0));

  alpha[4] = // alpha10
    n5 * ((     34729.0) / (    80640.0)) -
    n6 * ((   3418889.0) / (  1995840.0)) +
    n7 * ((  14644087.0) / (  9123840.0)) +
    n8 * ((2605413599.0) / (622702080.0));

  alpha[5] = // alpha12
    n6 * ((   212378941.0) / (  319334400.0)) -
    n7 * ((    30705481.0) / (   10378368.0)) +
    n8 * ((175214326799.0) / (58118860800.0));

  alpha[6] = // alpha14
    n7 * (( 1522256789.0) / (1383782400.0)) -
    n8 * ((16759934899.0) / (3113510400.0));

  alpha[7] = // alpha16
    n8 * ((1424729850961.0) / (743921418240.0));
  ...
  beta[1] = - // beta4
    ...
    n4 * ((     437.0) / (     1140.0)) -
    n5 * ((      46.0) / (      105.0)) +
    n6 * (( 1118711.0) / (  3870720.0)) -
    n7 * ((   51841.0) / (  1209600.0)) -
  ...
  beta[2] = - // beta6
    ...
    n5 * ((    209.0) / (    4480.0)) -
    n6 * ((   5569.0) / (   90720.0)) -
    n7 * ((9261899.0) / (58060800.0)) +
    n8 * ((6457463.0) / (17740800.0));

  beta[3] = - // beta8
    n4 * ((     4397.0) / (    161280.0)) +
    n5 * ((       11.0) / (       504.0)) +
    n6 * ((   830251.0) / (   7257600.0)) -
    n7 * ((   466511.0) / (   2494800.0)) -
    n8 * ((324154477.0) / (7664025600.0));

  beta[4] = - // beta10
    n5 * ((    4583.0) / (   161280.0)) +
    n6 * ((  108847.0) / (  3991680.0)) +
    n7 * (( 8005831.0) / ( 63866880.0)) -
    n8 * ((22894433.0) / (124540416.0));

  beta[5] = - // beta12
    n6 * ((  20648693.0) / (  638668800.0)) +
    n7 * ((  16363163.0) / (  518918400.0)) +
    n8 * ((2204645983.0) / (12915302400.0));
    
  beta[6] = - // beta14
    n7 * ((219941297.0) / ( 5535129600.0)) +
    n8 * ((497323811.0) / (12454041600.0));

  beta[7] = - // beta16
    n8 * ((191773887257.0) / (3719607091200.0));
}
